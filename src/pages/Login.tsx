...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err.code === 'auth/invalid-credential' || err.code === 'auth/wrong-password'
        ...
        ? 'تم تعطيل الحساب مؤقتاً. حاول لاحقاً أو أعد تعيين كلمة المرور'
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      setError('تعذّر إرسال رابط الاستعادة. تأكد من صحة البريد الإلكتروني');
    }
  };
    ...
    <div className="flex items-center justify-center min-h-[80vh] p-4">
      <div className="bg-white border border-border rounded-3xl p-8 max-w-md w-full shadow-2xl shadow-primary/5 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          {resetSent && (
            <div className="bg-emerald-50 text-emerald-700 p-4 rounded-2xl mb-5 text-sm font-semibold border border-emerald-100 text-right">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 bg-white border-2 border-border text-text font-bold py-3.5 rounded-2xl hover:border-primary/40 hover:bg-gray-50 transition-all active:scale-[0.98] disabled:opacity-60 shadow-sm mb-5 group"
          >
            {googleLoading ? (
          ...
          <div className="mt-7 text-center text-sm font-medium text-muted">
            ليس لديك حساب؟{' '}
